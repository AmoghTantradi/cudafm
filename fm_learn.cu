...
void fm_learn::learn(Data* train, Data* test) {
    ...
    DVector<sparse_row<DATA_FLOAT>> train_data;
    train_data.setSize(train->data->getNumRows());
    int idx = 0;
    for (train->data->begin(); !train->data->end(); train->data->next()) {
      sparse_row<DATA_FLOAT>& row = train->data->getRow();
      train_data(idx).size = row.size;
      cudaMalloc((void**) &train_data(idx).data, sizeof(DATA_FLOAT)*row.size);
      cudaMemcpy(train_data(idx).data, row.data, sizeof(DATA_FLOAT)*row.size, cudaMemcpyHostToDevice);
    }
    for (int i = 0; i < num_iter; i++) {
        ...
        for (train->data->begin(); !train->data->end(); train->data->next()) {
          ...
          Dvector<double> * sum;
          //cudaMalloc((void **) &sum, sizeof(Dvector<double>))
          SGD(train->data->getRow(), &train_data(i), mult, &sum);
          idx++;
        }
...
void fm_learn::SGD(sparse_row<DATA_FLOAT> *x, const double multiplier, DVector<double> *sum) {
  ...
  */
  cudaSGD<<<1, NUM_THREADS>>>(x, multiplier, learn_rate, fm, sum);
}
